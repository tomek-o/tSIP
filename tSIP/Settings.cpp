...
int Settings::Write(AnsiString asFileName)
{
	...
	try
	{
		...
		if (FileExists(asFileName))
		{
			...
			MoveFileEx(asFileName.c_str(), backupFile.c_str(), MOVEFILE_REPLACE_EXISTING | MOVEFILE_COPY_ALLOWED);
		}

		if (!MoveFileEx(tmpFile.c_str(), asFileName.c_str(), MOVEFILE_REPLACE_EXISTING | MOVEFILE_WRITE_THROUGH | MOVEFILE_COPY_ALLOWED))
		{
			DeleteFile(tmpFile.c_str());
			if (FileExists(backupFile))
			{
				MoveFileEx(backupFile.c_str(), asFileName.c_str(), MOVEFILE_REPLACE_EXISTING | MOVEFILE_COPY_ALLOWED);
			}
			return 1;
		}
	}
