...
#define DEBUG_MODULE "gzrtp_stream"
...
ZRTPConfig::ZRTPConfig(void)
{
	...
	start_parallel = cfg->zrtp.start_parallel;
	str_ncpy(zid_filename, cfg->zrtp.zid_filename, sizeof(zid_filename));
}
...
void SRTPStat::update(int ret_code, bool quiet)
{
	...
	case EALREADY:
		...
		break;
	default:
		DEBUG_WARNING("zrtp: %s unprotect failed: %m\n",
	...
	if (*burst == m_threshold) {
		*burst = 0;
		...
		if (!quiet)
			DEBUG_WARNING("zrtp: Stream <%s>: %s %s, %d packets\n",
				m_stream->media_name(),
	...
	, m_srtcp_stat(this, true, SRTP_ERR_BURST_THRESHOLD)
{
	...
	err = lock_alloc(&m_zrtp_lock);
	err |= lock_alloc(&m_send_lock);
	if (err)
		return;
	...
	if (!zf->isOpen()) {
		if (zf->open((char *)config.zid_filename) == -1) {
			DEBUG_WARNING("zrtp: Couldn't open/create ZID file %s\n",
			        config.zid_filename);
			err = ENOENT;
			return;
		}
...
int Stream::start(Stream *master)
{
	...
	DEBUG_INFO("zrtp: Starting <%s> stream%s\n", media_name(),
...
void Stream::stop()
{
	...
	if (!m_zrtp->isMultiStream() && m_recv_srtp && m_srtp_stat.ok() < 20) {
		...
		DEBUG_INFO("zrtp: Stream <%s>: received too few valid SRTP "
	...
	DEBUG_INFO("zrtp: Stopping <%s> stream\n", media_name());

	m_zrtp->stopZrtp();

	lock_write_get(m_send_lock);
	delete m_send_srtp;
	m_send_srtp = NULL;
	lock_rel(m_send_lock);

	delete m_recv_srtp;
	m_recv_srtp = NULL;

	DEBUG_INFO("zrtp: Stream <%s> stopped\n", media_name());
}
...
bool Stream::udp_helper_send(int *err, struct sa *src, struct mbuf *mb)
{
	...
	if (rerr) {
		DEBUG_WARNING("zrtp: protect/protect_ctrl failed (len=%u): %m\n",
		        len, rerr);
...
bool Stream::recv_zrtp(struct mbuf *mb)
{
	...
	if (size < ZRTP_MIN_PACKET_LENGTH) {
		DEBUG_WARNING("zrtp: incoming packet size (%d) is too small\n",
				size);
		return false;
	}
...
int32_t Stream::sendDataZRTP(const uint8_t* data, int32_t length)
{
	...
	if (err)
		DEBUG_WARNING("zrtp: udp_send_helper: %m\n", err);
...
bool Stream::srtpSecretsReady(SrtpSecret_t* secrets, EnableSecurity part)
{
	...
	DEBUG_INFO("zrtp: Stream <%s>: secrets are ready for %s\n",
	      media_name(),
	      (part == ForSender)? "sender" : "receiver");

	s = new Srtp(err, secrets, part);
	if (!s || err) {
		DEBUG_WARNING("zrtp: Stream <%s>: Srtp creation failed: %m\n",
		        media_name(), err);
		delete s;
		return false;
	}
...
{
	DEBUG_INFO("zrtp: Stream <%s>: secrets are off for %s\n",
	      media_name(),
	      (part == ForSender)? "sender" : "receiver");
...
void Stream::srtpSecretsOn(std::string c, std::string s, bool verified)
{
	...
	if (s.empty()) {
		DEBUG_WARNING("zrtp: Stream <%s> is encrypted (%s)\n",
			 media_name(), c.c_str());
	}
	else {
		DEBUG_WARNING("zrtp: Stream <%s> is encrypted (%s), "
	...
	#if 0
		if (!verified)
			DEBUG_WARNING("zrtp: SAS is not verified, type "
...
{
	lock_write_get(m_zrtp_lock);
}


void Stream::synchLeave()
{
	lock_rel(m_zrtp_lock);
}
