...
#pragma message "ZRTP SAS relay support is enabled."
    ...
    hashFunctionImpl = sha256; //static_cast<void (*)(const uint8_t *, uint64_t, uint8_t *)>(sha256);
...
ZrtpPacketConfirm* ZRtp::prepareConfirm2(ZrtpPacketConfirm* confirm1, uint32_t* errMsg) {
    ...
    uint32_t hmlen = (confirm1->getLength() - (unsigned int)9) * ZRTP_WORD_SIZE;
    ...
    hmlen = (zrtpConfirm2.getLength() - (unsigned int)9) * ZRTP_WORD_SIZE;
...
ZrtpPacketConf2Ack* ZRtp::prepareConf2Ack(ZrtpPacketConfirm *confirm2, uint32_t* errMsg) {
    ...
    uint32_t hmlen = (confirm2->getLength() - (unsigned int)9) * ZRTP_WORD_SIZE;
...
ZrtpPacketErrorAck* ZRtp::prepareErrorAck(ZrtpPacketError* epkt) {
    if (epkt->getLength() < 4)
        sendInfo(ZrtpError, CriticalSWError);
    else
        sendInfo(ZrtpError, epkt->getErrorCode());
    return &zrtpErrorAck;
}
...
void ZRtp::setNegotiatedHash(AlgorithmEnum* hash) {
    ...
    case 2:
        ...
        hashListFunction = skein256;//static_cast<void (*) (const std::vector<const uint8_t*>&, const std::vector<uint64_t>&, uint8_t *)>(skein256);

        hmacFunction = macSkein256;
        hmacListFunction = macSkein256;//static_cast<void (*)(const uint8_t*, uint64_t, const std::vector<const uint8_t*>&, const std::vector<uint64_t>&, uint8_t *, uint32_t *)>(macSkein256);
    ...
    case 3:
		...
		hashListFunction = skein384;//static_cast<void (*) (const std::vector<const uint8_t*>&, const std::vector<uint64_t>&, uint8_t *)>(skein384);

        hmacFunction = macSkein384;
        hmacListFunction = macSkein384;//static_cast<void (*)(const uint8_t*, uint64_t, const std::vector<const uint8_t*>&, const std::vector<uint64_t>&, uint8_t *, uint32_t *)>(macSkein384);
...
bool ZRtp::sendSASRelayPacket(uint8_t* sh, std::string render) {
    ...
    uint32_t hmlen = (zrtpSasRelay.getLength() - (unsigned int)9) * ZRTP_WORD_SIZE;
...
bool ZRtp::checkAndSetNonce(uint8_t* nonce) {
	...
	for (unsigned int i= 0; i<masterStream->peerNonces.size(); i++) {
		const std::string& usedNonce = masterStream->peerNonces[i];
