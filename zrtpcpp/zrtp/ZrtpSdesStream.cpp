...
static suiteParam knownSuites[] = {
	...
	{(ZrtpSdesStream::sdesSuites)0, NULL, 0, 0, 0, NULL, NULL, 0, 0, 0}
};
...
ZrtpSdesStream::ZrtpSdesStream(const sdesSuites s) :
	...
	sendSrtcp(NULL), srtcpIndex(0), recvZrtpTunnel(NULL), sendZrtpTunnel(NULL), cryptoMixHashLength(0),
...
void ZrtpSdesStream::computeMixedKeys(bool sipInvite) {
	...
	expand(prk, prkLen, NULL, 0U, L, cryptoMixHashLength/8, T);
